...
{
    public class CancelGhnOrderConsumer : IConsumer<CancelGhnOrderMessage>
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<CancelGhnOrderConsumer> _logger;
        private readonly HttpClient _httpClient;

        public CancelGhnOrderConsumer(
            IConfiguration configuration,
            ILogger<CancelGhnOrderConsumer> logger)
        {
        ...
        public async Task Consume(ConsumeContext<CancelGhnOrderMessage> context)
        {
            var message = context.Message;
            _logger.LogInformation("Consuming CancelGhnOrderMessage for Order Code: {OrderCode}, GHN Code: {GhnCode}", 
            ...
            // 2. Call GHN Cancel Shipping Order API
            var ghnPayload = new
            {
                order_codes = new[] { message.GhnOrderCode }
            };

            var url = $"{baseUrl}/shiip/public-api/v2/shipping-order/cancel";
            ...
            try
            {
                ...
                {
                    _logger.LogError("GHN Order Cancellation failed with HTTP code {StatusCode}. Response: {Response}", response.StatusCode, responseBody);
                ...
                if (code != 200)
                {
                    ...
                    _logger.LogError("GHN Order Cancellation failed with business error. Response: {Response}", responseBody);
                    throw new Exception($"GHN Business Error: {errMsg}");
                }

                _logger.LogInformation("Successfully cancelled GHN Shipping Order for Order Code: {OrderCode}, GHN Code: {GhnCode}", 
                    message.OrderCode, message.GhnOrderCode);
            }
            catch (Exception ex)
            {
                _logger.LogError("Error cancelling GHN order: {GhnCode}. Details: {Message}", message.GhnOrderCode, ex.Message);
                throw;
            }
        }
    }
